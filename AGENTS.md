...
Juego 2D top-down en C++17/raylib. Versión actual: **v0.1.0 — "La última transmisión"** (ver `vcpkg.json`, `README.md`).

- Escenario: Neo-Córdoba 2077, "La ciudad que nos olvidó".
- El jugador elige entre **Emmanuel "Emma" Vázquez** (intrusión de sistemas / hacking) o **Magalí "Magga" Temerro** (combate de proximidad). La elección cambia la ruta del prólogo; ambas rutas convergen en la entrada clausurada del Neometro.
...
- Objetivo de v0.1.0: prólogo jugable completo (selección de personaje → intro → exterior → ruta de habilidad → entrada Neometro → teaser de la próxima versión). v0.2.0 ("Neometro: El último convoy") es **planeado**, no implementado — incluirá las estructuras Queue/Stack académicas sin STL (`docs/roadmap.md`).
...
| `SceneStack` | Implementado, es el activo en `cyber_cba` | `src/ui/SceneStack.{hpp,cpp}` | Contrato de escenas apilables (`IStackScene`: `updatesBelow()`, `drawsBelow()`, `update()`, `draw()`). Compilado en el ejecutable real. |
| `SceneManager` / `IScene` | **No usado** (código muerto) | `src/ui/SceneManager.{hpp,cpp}`, `src/ui/IScene.hpp` | Existe en el árbol pero **no está en las fuentes de `cyber_cba` en `CMakeLists.txt`**. No lo referencies como el flujo activo. |
