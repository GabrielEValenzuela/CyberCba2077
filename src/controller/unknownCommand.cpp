...
namespace CyberpunkCba
{
    ...
    UnknownCommand::UnknownCommand(
        std::string input, const CommandRegistry& registry) // actualizar el constructor para recibir la referencia a la
                                                            // tabla de comandos, y guardarla en el atributo m_registry.
    ...
    void UnknownCommand::execute(GameModel& /*model*/)
    {
        ...
        if (m_input.size() >= 2)
        {
            ...
            for (const Command* cmd : m_registry.commands())
            {                                            // por cada puntero cmd a la lista de comanddos del registry
                std::string nombreComando = cmd->name(); // guardo el nombre del comando :P
                if (nombreComando.size() >= 2 && nombreComando.substr(0, 2) == sugerencia)
                { // nos aseguramos que el comando tenga almenos dos caracteres y los comparamos con el de la sugerencia
